...
public class FlameItem implements StaticEntity {
  ...
  public FlameItem(double x, double y) {
    ...
    layer = 0;
    sprite =
        new Sprite(this, GlobalConstants.itemImage, 64, 0.1, 0, 64, 4, width, height, scale, true);
    entityBoundary =
        new RectBoundedBox(
  ...
  public void update() {
    ...
    if (isColliding(p)) {
      GameVariables.sizeOfExploding += 1;
      Game.getLabelList().get(5).setText(Integer.toString(GameVariables.sizeOfExploding));
      try {
        Game.playSound(2);
  ...
  public void removeFromScene() {
    Game.getFlameItems().remove(this);
